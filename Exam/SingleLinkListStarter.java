...
public class SingleLinkList {
    ...
    public void delete_greater(int number)
    {
        ...
        //before is initialized to the first element in the list 
        //after is 
        node after, before;

        if(start == null) {

            return; //method immedietly returns if head is null

        } else {
            after = start; //after --> first node in the list
            before = null; //before is initialized to the first element
            while(after != null) { //loops through each element in the list
                if(after.data > number) //if after.data > number, after is removed 
                {
                    if(before == null) { //is before == null, after --> first node
                        start = after.next; //head is updated to the next node
                    } else {
                        before.next = after.next; //deletes current node
                    }
                } else {
                    before = after; //no deletion occurs and we move on
                }
                after = after.next; 
            }
        }
